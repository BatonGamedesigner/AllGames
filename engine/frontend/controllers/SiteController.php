...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $furniture = StaticText::find()->where(['id'=>StaticText::FURNITURE])->one();
        $furniture['texts'] = $furniturePhotos = [];
        $furniture['texts'] = StaticTextItem::find()->where(['id'=>[StaticTextItem::FURNITURE_TEXT, StaticTextItem::LINK]])->indexBy('id')->all();
        foreach ($furniture->getPhotos()->orderBy(['pos'=>SORT_DESC])->all() as $photo) {
            /* @var $photo StaticTextPhoto */
            $photoBuf = [];
            $photoBuf['src'] = $photoBuf['title'] = null;
            $photoBuf['src'] = $photo->getSRCPhoto(['parent_id' => $furniture->id, 'suffix' => '_big']);
            $photoBuf['title'] = (!empty($photo->name))?$photo->name:'Фото';
            $furniturePhotos['data'][] = $photoBuf;
            $photoBuf['src_sm'] = $photo->getSRCPhoto(['parent_id' => $furniture->id, 'suffix' => '_sm']);
            $furniturePhotos['preview'][] = $photoBuf;
        ...
        $reviews = Review::find()->where(['active'=>1])->orderBy(['pos'=>SORT_DESC])->all();
...
//        }
//        else
        return $this->render('index',compact('callback','staticText','categories', 'banners', 'furniture','furniturePhotos','collections','products', 'reviews'));
    }
